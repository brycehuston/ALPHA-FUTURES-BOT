...
def generate_signal(candles: Sequence[Candle], settings: StrategySettings | None = None) -> Signal:
    ...
    regime = detect_regime(snapshot, strategy_settings)
    ...
    if regime is Regime.BULL_TREND:
        score = _score_long(snapshot, last_candle, strategy_settings)
        if score >= strategy_settings.min_signal_score:
            ...
            return Signal(
                ...
                stop_loss=entry_price - (strategy_settings.stop_loss_atr_multiplier * snapshot.atr),
                take_profit=entry_price + (strategy_settings.take_profit_atr_multiplier * snapshot.atr),
            )
        return _no_trade("long pullback score below threshold", score)

    if regime is Regime.BEAR_TREND:
        score = _score_short(snapshot, last_candle, strategy_settings)
        if score >= strategy_settings.min_signal_score:
            entry_price = snapshot.last_close
            return Signal(
                symbol=Symbol.BTC,
...
def _score_long(snapshot: IndicatorSnapshot, candle: Candle, settings: StrategySettings) -> float:
    score = 0
    pullback_band = settings.pullback_atr_tolerance * snapshot.atr
    if snapshot.last_close > snapshot.ema_trend and snapshot.ema_fast > snapshot.ema_slow:
        score += TREND_ALIGNMENT_POINTS
    if snapshot.last_close >= snapshot.vwap - (settings.vwap_near_atr_multiplier * snapshot.atr):
    ...
    if not settings.require_volume_confirmation or snapshot.last_volume >= snapshot.volume_average:
        score += VOLUME_CONFIRMATION_POINTS
    if not settings.require_candle_confirmation or candle.close > candle.open:
        score += CANDLE_CONFIRMATION_POINTS
    return _clamp_score(score)


def _score_short(snapshot: IndicatorSnapshot, candle: Candle, settings: StrategySettings) -> float:
    score = 0
    pullback_band = settings.pullback_atr_tolerance * snapshot.atr
    if snapshot.last_close < snapshot.ema_trend and snapshot.ema_fast < snapshot.ema_slow:
        score += TREND_ALIGNMENT_POINTS
